...
# doing the thansform to the image
def PhotometricDistortion(threhold,input):
    '''
    - threhold: if x > threhold, we transform the input
    '''

    random_number = random.random()
    if random_number > 0.5:
        T.ColorJitter(brightness=(2, 2), contrast=(0.5, 0.5), saturation=(0.5, 0.5))(input)
    return input

# add the guassion noise to the image
def guassionAugenmantion(inputs, noise_factor=0.3):
    random_number = random.random()
    if random_number > 0.5:
        output = inputs + torch.randn_like(inputs) * noise_factor
    noisy = torch.clip(output, 0., 1.)
    return noisy

# add random box patch to the image
